...
interface CanvasContainerProps {
  refs: WaveParameter;
  other_refs: Wave;
  controllable_refs : controllables;
  fields: WaveParameter[];
}

const Canvascontainer: React.FC<CanvasContainerProps> = ({refs,other_refs,fields,controllable_refs}) => {
  ...
  while (tElapsed <= totalTime) {
      ...
      for (let t = tElapsed; t < tEnd; t += dt) {
          ...
          pts.push({
              ...
              y: y0 - v * parseFloat(controllable_refs.PIXELS_PER_MV.current.value)
          });
      }
